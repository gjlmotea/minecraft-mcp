...
export const blockNameSchema = z
  ...
  .min(1)
  .max(80)
  .describe('方塊 ID，例如 stone 或 minecraft:oak_planks');

export const blockStatesSchema = z
  .string()
  .trim()
  .min(2)
  .max(200)
  .describe('選用的方塊狀態，例如 ["stone_type":"granite"]');
...
export const selectorSchema = z
  ...
  .min(1)
  .max(120)
  .describe('目標選擇器（@a/@p/@s/@e/@r，可帶 []）或玩家名稱');
...
export const batchOutcomeSchema = z
  .object({
    ok: z.boolean(),
    issued: z.number(),
    succeeded: z.number(),
...
export const shapeSchema = z.discriminatedUnion('kind', [
  ...
  z
    .object({ kind: z.literal('box'), from: vec3Schema, to: vec3Schema, hollow: z.boolean().default(false) })
    .strict(),
  z
    .object({
  ...
  z
    .object({
      ...
      center: vec3Schema,
      radii: z.object({ x: z.number().min(1).max(128), y: z.number().min(1).max(128), z: z.number().min(1).max(128) }).strict(),
      hollow: z.boolean().default(false),
    })
    .strict(),
  z
...
export const agentProgramStepSchema = z.discriminatedUnion('action', [
  z
    .object({
